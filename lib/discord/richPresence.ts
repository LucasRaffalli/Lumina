...
let currentPartyId = ''; // Unique pour chaque session
...
async function attemptConnection(clientId: string, profile?: Partial<RPC.Presence>) {
    try {
        ...
        // Nettoyage de la connexion précédente si elle existe
        await cleanup();
            ...
            rpc!.once('ready', async () => {
                ...
                if (profile) {
                    const updates = {
                        ...
                        startTimestamp: profile.startTimestamp || (profile.timestamp ? new Date(profile.timestamp).getTime() : undefined),
